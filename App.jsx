...
import { 
  ...
  Plus, X, Loader2, Sliders, Wand2, Info, Grid, RotateCw, Image as ImageIcon
} from 'lucide-react';

// --- 1. STILI GLOBALI E RESET ---
...
// --- 3. STILI CSS-IN-JS (Adattati per Mobile Web & Desktop) ---
const s = {
  container: { 
    display: 'flex', flexDirection: 'column', height: '100vh', 
    backgroundColor: colors.bg, color: colors.text, 
    fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif', 
    paddingTop: 'env(safe-area-inset-top)', overflow: 'hidden',
    // Simulazione Mobile su Desktop
  ...
  main: { flex: 1, overflowY: 'auto', padding: '12px', paddingBottom: '220px' }, // Extra padding per il footer alto
  ...
  emptyState: { marginTop: '40px', border: `2px dashed ${colors.border}`, borderRadius: '24px', padding: '40px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px', backgroundColor: 'rgba(39,39,42,0.2)', cursor: 'pointer' },

  // Footer Complesso (Controlli)
  footer: { 
    ...
    paddingBottom: 'env(safe-area-inset-bottom)', zIndex: 40, boxShadow: '0 -10px 40px rgba(0,0,0,0.8)' 
  },
  ...
  controlArea: { padding: '16px', height: '180px', overflowY: 'auto' }, // Area scrollabile per i controlli
  actionRow: { display: 'flex', gap: '8px', padding: '12px 16px', backgroundColor: colors.bg, borderTop: `1px solid ${colors.border}` },
  ...
  // Posizionamento Griglia (Watermark)
...
export default function App() {
  ...
  // Riferimenti agli input file nascosti
  const fileInputRef = useRef(null);
  const watermarkInputRef = useRef(null);

  // Stato Completo (Tutte le funzioni Android)
  ...
  // Init (Mock Privacy Check su Web)
  useEffect(() => {
    const checkPrivacy = () => {
       const accepted = localStorage.getItem('privacy_accepted');
       // Qui potresti mostrare un modal se serve
    };
    checkPrivacy();
  }, []);

  // Trigger Automatico Elaborazione
  useEffect(() => {
    if (images.length === 0) return;
    // Debounce per evitare troppi ricalcoli mentre sposti gli slider
    const timer = setTimeout(() => {
        if(!isProcessing) processAllLive();
    }, autoTrigger ? 50 : 500);
    return () => clearTimeout(timer);
  }, [settings, images.length]);

  // Trigger Salvataggio Automatico (se attivo)
  useEffect(() => {
  ...
  const handleAutoMagic = () => {
      setSettings(prev => ({ 
          ...prev, 
          aiEnhance: true, aiIntensity: 60, 
          exposureEnabled: true, exposure: 10, 
          contrastEnabled: true, contrast: 10,
          watermarkEnabled: true // Attiva watermark se presente
      }));
      setAutoTrigger(true);
  };
  ...
  // --- MOTORE DI ELABORAZIONE (Canvas API) ---
  ...
  const processSingleImage = (imgObj) => {
    return new Promise((resolve, reject) => {
      ...
      img.crossOrigin = "Anonymous"; // Cruciale per il web
      img.onload = () => {
        try {
            ...
            // Limite di sicurezza per browser mobile (evita crash memoria)
            const MAX_SAFE = 3000;
            if (w > MAX_SAFE || h > MAX_SAFE) { const r = Math.min(MAX_SAFE/w, MAX_SAFE/h); w *= r; h *= r; }

            // 1. RESIZE & UPSCALE logic
            if (settings.upscale) { w *= 2; h *= 2; }
            if (settings.resizeEnabled) {
                if (settings.resizeMode === 'percent') { 
                    w *= (settings.widthPercent/100); 
                    h *= (settings.widthPercent/100); 
                } else {
                    // Fit logic
            ...
            if (settings.exposureEnabled) b += (settings.exposure / 2); // Exposure logic
                ...
                const drawContent = (x, y) => {
                    ...
                    if (settings.watermarkType === 'text' && settings.watermarkText) {
                        ...
                        ctx.fillText(settings.watermarkText, 0, 0);
                         ...
                         // Disegna immagine centrata nel punto
                         try { ctx.drawImage(wmImg, -size/2, -size/2, size, size); } catch(e){}
                    }
                    ctx.restore();
                };
  ...
  // --- DOWNLOAD MANAGER (Web Specific) ---
  const saveAllToDevice = async () => {
    let count = 0;
    // Su web non possiamo salvare silenziosamente, dobbiamo triggerare il download
    // Per UX migliore, facciamo un piccolo delay tra i download se sono tanti
    for (const img of images) {
        ...
        const name = (settings.renameEnabled ? settings.prefix : 'Img_') + Date.now() + '_' + count + '.' + ext;
        ...
        count++;
        // Piccolo delay per non bloccare il browser
        await new Promise(r => setTimeout(r, 200));
    }
    
    // Fallback alert (su Android nativo useremmo Toast)
    if(count > 0) alert(`Download started for ${count} images! Check your downloads folder.`);
  };
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.main}>
        {images.length === 0 ? (
            <div onClick={handleGalleryUpload} style={s.emptyState}>
                <Upload size={32} color={colors.primary}/>
                <div style={{textAlign:'center', color:colors.textMuted}}>
                    <p style={{color:'#fff', margin:0, fontSize:'14px'}}>Click to Upload</p>
                    <small style={{fontSize:'10px'}}>JPG, PNG, WEBP supported</small>
                </div>
            </div>
        ) : (
            <div style={s.grid}>
      ...
      {/* HIDDEN INPUTS */}
      <input type="file" ref={fileInputRef} onChange={handleFileInput} multiple style={{display:'none'}} accept="image/*" />
      ...
      {images.length > 0 && appMode === 'quick' && (
        <div style={s.footer}>
            ...
            <div style={s.controlArea}>
                ...
                {activeTab === 'enhance' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                        ...
                        <div>
                             <div style={s.row}>
                                <span style={s.label}>Contrast</span>
                                <SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.contrastEnabled} onChange={v => setSettings({...settings, contrastEnabled:v})} />
                            </div>
                            {settings.contrastEnabled && <input type="range" min="-50" max="50" value={settings.contrast} onChange={e=>setSettings({...settings, contrast:parseInt(e.target.value)})} style={s.range} />}
                        </div>
                        
                        <div style={s.row}>
                             <span style={s.label}>B/W Filter</span>
                             <SegmentedToggle options={[{label:'NO', value:false}, {label:'YES', value:true}]} value={settings.grayscale} onChange={v => setSettings({...settings, grayscale:v})} />
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'resize' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                         <div style={s.row}>
                            <span style={s.label}>Enable Resize</span>
                            <SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.resizeEnabled} onChange={v => setSettings({...settings, resizeEnabled:v})} />
                        </div>
                        {settings.resizeEnabled && (
                            <>
                                <div style={{marginBottom:'8px'}}>
                                    <SegmentedToggle options={[{label:'Percent %', value:'percent'}, {label:'Fixed PX', value:'fixed'}]} value={settings.resizeMode} onChange={v => setSettings({...settings, resizeMode:v})} />
                                </div>
                                {settings.resizeMode === 'percent' ? (
                                    <input type="range" min="10" max="100" value={settings.widthPercent} onChange={e=>setSettings({...settings, widthPercent:parseInt(e.target.value)})} style={s.range} />
                                ) : (
                                    <div style={{display:'flex', gap:'8px'}}>
                                        <input type="number" placeholder="W" value={settings.targetWidthPx} onChange={e=>setSettings({...settings, targetWidthPx:parseInt(e.target.value)})} style={s.input} />
                                        <input type="number" placeholder="H" value={settings.targetHeightPx} onChange={e=>setSettings({...settings, targetHeightPx:parseInt(e.target.value)})} style={s.input} />
                                    </div>
                                )}
                            </>
                        )}
                        <div style={s.row}>
                            <span style={s.label}>Smart Upscale (2x)</span>
                            <SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.upscale} onChange={v => setSettings({...settings, upscale:v})} />
                        </div>
                    </div>
                )}
                ...
                {activeTab === 'watermark' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                         <div style={s.row}>
                            <span style={s.label}>Watermark</span>
                            <SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.watermarkEnabled} onChange={v => setSettings({...settings, watermarkEnabled:v})} />
                        </div>
                        {settings.watermarkEnabled && (
                            <>
                                <div style={{marginBottom:'8px'}}>
                                    <SegmentedToggle options={[{label:'TEXT', value:'text'}, {label:'IMAGE', value:'image'}]} value={settings.watermarkType} onChange={v => setSettings({...settings, watermarkType:v})} />
                                </div>
                                
                                {settings.watermarkType === 'text' ? (
                                    <>
                                        <div style={{display:'flex', gap:'8px'}}>
                                            <input type="text" value={settings.watermarkText} onChange={e=>setSettings({...settings, watermarkText:e.target.value})} style={{...s.input, flex:2}} placeholder="Your Text..." />
                                            <input type="color" value={settings.watermarkColor} onChange={e=>setSettings({...settings, watermarkColor:e.target.value})} style={{flex:1, height:'38px', background:'none', border:'none', cursor:'pointer'}} />
                                        </div>
                                        <div style={{display:'flex', gap:'4px', overflowX:'auto', paddingBottom:'4px'}}>
                                            {['sans-serif', 'serif', 'monospace', 'cursive'].map(f => (
                                                <button key={f} onClick={()=>setSettings({...settings, watermarkFont:f})} style={{padding:'4px 8px', border:`1px solid ${colors.border}`, background: settings.watermarkFont===f ? colors.primary : 'transparent', color: settings.watermarkFont===f ? colors.black : colors.text, borderRadius:'4px', fontSize:'10px', cursor:'pointer'}}>{f.slice(0,4)}</button>
                                            ))}
                                        </div>
                                    </>
                                ) : (
                                    <button onClick={()=>watermarkInputRef.current.click()} style={{...s.input, borderStyle:'dashed', textAlign:'center', color:colors.textMuted, cursor:'pointer'}}>
                                        {settings.watermarkImage ? 'Image Loaded (Click to Change)' : 'Click to Upload Logo (PNG)'}
                                    </button>
                                )}

                                <div style={s.row}>
                                    <span style={s.label}><Grid size={12} style={{marginRight:4}}/> Tile Pattern</span>
                                    <SegmentedToggle options={[{label:'NO', value:false}, {label:'YES', value:true}]} value={settings.watermarkTile} onChange={v => setSettings({...settings, watermarkTile:v})} />
                                </div>

                                <div style={{display:'flex', justifyContent:'space-between', marginTop:'8px'}}>
                                    {!settings.watermarkTile && (
                                        <div style={s.posGrid}>
                                            {['top-left','top-center','top-right','center-left','center','center-right','bottom-left','bottom-center','bottom-right'].map(p => (
                                                <button key={p} onClick={()=>setSettings({...settings, watermarkPosition:p})} style={s.posCell(settings.watermarkPosition===p)} />
                                            ))}
                                        </div>
                                    )}
                                    <div style={{flex:1, marginLeft: settings.watermarkTile ? 0 : '12px'}}>
                                        <span style={s.label}>Opacity</span>
                                        <input type="range" min="10" max="100" value={settings.watermarkOpacity} onChange={e=>setSettings({...settings, watermarkOpacity:parseInt(e.target.value)})} style={s.range} />
                                        <span style={s.label}>Size</span>
                                        <input type="range" min="5" max="100" value={settings.watermarkSize} onChange={e=>setSettings({...settings, watermarkSize:parseInt(e.target.value)})} style={s.range} />
                                        <span style={s.label}><RotateCw size={10}/> Rotate</span>
                                        <input type="range" min="0" max="360" value={settings.watermarkRotation || 0} onChange={e=>setSettings({...settings, watermarkRotation:parseInt(e.target.value)})} style={s.range} />
                                    </div>
                                </div>
                            </>
                        )}
                ...
                {activeTab === 'output' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                        ...
                        <SegmentedToggle options={[{label:'JPG', value:'image/jpeg'}, {label:'PNG', value:'image/png'}, {label:'WEBP', value:'image/webp'}]} value={settings.format} onChange={v => setSettings({...settings, format:v})} />
                        
                        <div style={{marginTop:'8px'}}>
                            <span style={s.label}>Quality ({Math.round(settings.quality*100)}%)</span>
                            <input type="range" min="1" max="100" value={settings.quality*100} onChange={e=>setSettings({...settings, quality:parseInt(e.target.value)/100})} style={s.range} />
                        </div>
                        
                        <div style={{marginTop:'8px'}}>
                            <span style={s.label}>File Prefix</span>
                            <input type="text" value={settings.prefix} onChange={e=>setSettings({...settings, prefix:e.target.value})} style={s.input} />
                        </div>
                    </div>
                )}
            </div>
            ...
            <div style={s.actionRow}>
                ...
                <button onClick={handleAutoMagic} disabled={isProcessing} style={{...s.btn, width:'64px', background:'rgba(16,185,129,0.15)', border:`1px solid ${colors.primary}`, color:colors.primary, flexDirection:'column', gap:'0', padding:'0'}}>
                    <Wand2 size={16}/><span style={{fontSize:'8px', marginTop:'2px'}}>AUTO</span>
                </button>
                <button onClick={saveAllToDevice} disabled={isProcessing} style={{...s.btn, flex:1, background:colors.white, color:colors.black, minWidth:'120px'}}>
                    {isProcessing ? <Loader2 className="animate-spin"/> : <Download size={18}/>} <span style={{marginLeft:'4px'}}>SAVE BATCH</span>
                </button>
            </div>
        </div>
      )}
      ...
      {showInfo && (
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center'}} onClick={()=>setShowInfo(false)}>
             <div style={{background:colors.surface, padding:'24px', borderRadius:'16px', border:`1px solid ${colors.border}`, width:'80%', maxWidth:'300px', textAlign:'center'}} onClick={e=>e.stopPropagation()}>
                 ...
                 <p style={{color:colors.textMuted, fontSize:'12px', margin:'8px 0'}}>Web Version 1.2</p>
                 <button onClick={()=>setShowInfo(false)} style={{background:'none', border:'none', color:colors.white, textDecoration:'underline', fontSize:'12px', cursor:'pointer'}}>Close</button>
             </div>
        </div>
      )}
    </div>
