...
import { 
  Upload, Zap, Settings, RefreshCw, Type, Download, 
  Plus, X, Loader2, Sliders, Wand2, Info, Grid, RotateCw, Image as ImageIcon
} from 'lucide-react';

// --- STILI GLOBALI INIETTATI ---
// Serve per pulire i margini e nascondere le scrollbar del browser
const globalStyles = `
...
const s = {
  ...
  main: { flex: 1, overflowY: 'auto', padding: '12px', paddingBottom: '220px' },
  ...
  controlArea: { padding: '16px', height: '180px', overflowY: 'auto' },
...
export default function App() {
  ...
  // Init
  useEffect(() => {
    // Controllo localStorage invece di Preferences nativo
    const checkPrivacy = () => { try { localStorage.getItem('privacy_accepted'); } catch(e){} };
    checkPrivacy();
  }, []);
  ...
  const handleFileInput = (e) => {
    if (e.target.files) {
        const newImages = Array.from(e.target.files).map(f => ({
          ...
          status: 'pending', processedUrl: null, file: f
        }));
  ...
  const saveAllToDevice = async () => {
    ...
    for (const img of images) {
        ...
        count++;
        // Piccolo ritardo per non bloccare il browser
    ...
    <div style={s.container}>
      ...
      <div style={s.header}>
        ...
        <div style={{display:'flex', gap:'10px', alignItems:'center'}}>
             {images.length > 0 && <button onClick={handleGalleryUpload} style={{border:`1px solid ${colors.border}`, background:colors.surface, color:colors.primary, borderRadius:'6px', padding:'4px 8px', fontSize:'10px', fontWeight:'bold', cursor:'pointer', display:'flex', alignItems:'center', gap:'4px'}}><Plus size={12}/> ADD</button>}
            ...
            <div style={s.grid}>
                {images.map(img => (
                    <div key={img.id} style={s.card}>
                        ...
                        {img.status === 'done' && img.processedUrl && <div style={s.splitRight}><img src={img.processedUrl} style={s.imgSplit} alt="edit" /></div>}
      ...
      {images.length > 0 && appMode === 'quick' && (
        <div style={s.footer}>
            ...
            <div style={s.controlArea}>
                {activeTab === 'enhance' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                        <div style={s.row}><span style={s.label}><Zap size={10} style={{marginRight:4}}/> AI Auto Color</span><SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.aiEnhance} onChange={v => setSettings({...settings, aiEnhance:v})} /></div>
                        ...
                        <div style={s.row}><span style={s.label}>Exposure</span><SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.exposureEnabled} onChange={v => setSettings({...settings, exposureEnabled:v})} /></div>
                ...
                {activeTab === 'resize' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                         <div style={s.row}><span style={s.label}>Enable Resize</span><SegmentedToggle options={[{label:'OFF', value:false}, {label:'ON', value:true}]} value={settings.resizeEnabled} onChange={v => setSettings({...settings, resizeEnabled:v})} /></div>
                         {settings.resizeEnabled && settings.resizeMode === 'percent' && <input type="range" min="10" max="100" value={settings.widthPercent} onChange={e=>setSettings({...settings, widthPercent:parseInt(e.target.value)})} style={s.range} />}
                    </div>
                )}
                {activeTab === 'watermark' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                         ...
                         {settings.watermarkEnabled && (
                            <>
                                <div style={{marginBottom:'8px'}}><SegmentedToggle options={[{label:'TEXT', value:'text'}, {label:'IMAGE', value:'image'}]} value={settings.watermarkType} onChange={v => setSettings({...settings, watermarkType:v})} /></div>
                                {settings.watermarkType === 'text' ? <input type="text" value={settings.watermarkText} onChange={e=>setSettings({...settings, watermarkText:e.target.value})} style={s.input} /> : <button onClick={()=>watermarkInputRef.current.click()} style={{...s.input, borderStyle:'dashed', cursor:'pointer'}}>{settings.watermarkImage ? 'Image Loaded' : 'Upload Logo'}</button>}
                                <div style={s.row}><span style={s.label}>Tile</span><SegmentedToggle options={[{label:'NO', value:false}, {label:'YES', value:true}]} value={settings.watermarkTile} onChange={v => setSettings({...settings, watermarkTile:v})} /></div>
                            </>
                         )}
                    </div>
                )}
                {activeTab === 'output' && (
                    <div style={{display:'flex', flexDirection:'column', gap:'12px'}}>
                        <span style={s.label}>Format</span><SegmentedToggle options={[{label:'JPG', value:'image/jpeg'}, {label:'PNG', value:'image/png'}]} value={settings.format} onChange={v => setSettings({...settings, format:v})} />
                        <div style={{marginTop:'8px'}}><span style={s.label}>Prefix</span><input type="text" value={settings.prefix} onChange={e=>setSettings({...settings, prefix:e.target.value})} style={s.input} /></div>
                    </div>
                )}
            </div>
            ...
            <div style={s.actionRow}>
                ...
                <button onClick={saveAllToDevice} disabled={isProcessing} style={{...s.btn, flex:1, background:colors.white, color:colors.black, minWidth:'120px'}}>{isProcessing ? <Loader2 className="animate-spin"/> : <Download size={18}/>} <span style={{marginLeft:'4px'}}>SAVE BATCH</span></button>
            </div>
        </div>
      )}
      ...
      {showInfo && (
        <div style={{position:'fixed', inset:0, background:'rgba(0,0,0,0.8)', zIndex:100, display:'flex', alignItems:'center', justifyContent:'center'}} onClick={()=>setShowInfo(false)}>
             <div style={{background:colors.surface, padding:'24px', borderRadius:'16px', border:`1px solid ${colors.border}`, width:'80%', maxWidth:'300px', textAlign:'center'}} onClick={e=>e.stopPropagation()}>
                 ...
                 <p style={{color:colors.textMuted, fontSize:'12px', margin:'8px 0'}}>Web Version 1.3</p>
