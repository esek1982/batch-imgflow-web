...
import { 
  ...
  Plus, X, Loader2, Sliders, Wand2, Info, Grid, RotateCw
} from 'lucide-react';
...
export default function App() {
  ...
  const [settings, setSettings] = useState({
    ...
    watermarkEnabled: false, watermarkText: '© BatchFlow', watermarkOpacity: 50,
    format: 'image/jpeg', quality: 0.9, prefix: 'Batch_'
  });
  ...
  const handleFileInput = (e) => {
    if (e.target.files) {
        const newImages = Array.from(e.target.files).map(f => ({
          ...
          preview: URL.createObjectURL(f), status: 'pending', processedUrl: null, file: f
        }));
  ...
  const handleAutoMagic = () => {
      setSettings(prev => ({ ...prev, aiEnhance: true, aiIntensity: 60, exposureEnabled: true, exposure: 10 }));
      setAutoTrigger(true);
  };
  ...
  // PROCESSOR
  useEffect(() => {
    if (images.length > 0 && autoTrigger && !isProcessing) {
        processAllLive();
    }
  }, [autoTrigger]);
  ...
  const processAllLive = async () => {
    ...
    setImages(processed); setIsProcessing(false); setAutoTrigger(false);
  };
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        try {
            ...
            const MAX = 2500; // Web optimization
            ...
            if (settings.watermarkEnabled && settings.watermarkText) {
                ...
                ctx.fillText(settings.watermarkText, w - (w*0.15), h - (h*0.05));
                ctx.globalAlpha = 1.0;
            }
  ...
  // DOWNLOAD FUNCTION (Web specific)
  const saveAllToDevice = async () => {
     ...
     for (const img of images) {
         ...
         link.download = `${settings.prefix}${Date.now()}_${count}.jpg`;
         document.body.appendChild(link);
         link.click();
         document.body.removeChild(link);
         await new Promise(r => setTimeout(r, 200)); // Delay to prevent browser throttling
         count++;
     }
     alert(`Downloaded ${count} images!`);
  };
                ...
                {images.map(img => (
                    <div key={img.id} style={s.card}>
                        ...
                        {img.status === 'done' && img.processedUrl && (
                            <div style={s.splitRight}><img src={img.processedUrl} style={s.imgSplit} alt="edit" /></div>
                        )}
